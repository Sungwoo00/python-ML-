...
def record_accuracy(stage, x_train_processed, x_valid_processed, x_test_processed):
    classifiers = {
    ...
    accuracy_results_valid.append((stage, valid_accuracies))
    accuracy_results_test.append((stage, test_accuracies))
...
stages = [result[0] for result in accuracy_results_valid]
...
for i, name in enumerate(classifiers):
    ...
    valid_accuracies = [result[1][name] for result in accuracy_results_valid]
    test_accuracies = [result[1][name] for result in accuracy_results_test]
    
    plt.plot(stages, valid_accuracies, marker='o', linestyle='-', label='Valid Accuracy')
    plt.plot(stages, test_accuracies, marker='x', linestyle='--', label='Test Accuracy')

    plt.xlabel('Preprocessing Stage')
    plt.ylabel('Accuracy')
    plt.title(f'{name} - Accuracy by Preprocessing Stage')
    plt.legend()
    plt.grid(True)
...
# Train 및 Valid 정확도 표 생성
plt.figure(figsize=(10, 4))
plt.axis('off')

# 표 내용 생성
table_data = [['Preprocessing Stage'] + classifiers * 2]
table_data[0].insert(1, ' ')
table_data[0].insert(len(classifiers) + 2, ' ')

for j, stage in enumerate(stages):
    row = [stage, ' ']  # stage 열과 빈 칸
    for name in classifiers:
        row.append(accuracy_results_valid[j][1][name])
    row.append(' ')  # 빈 칸 추가
    for name in classifiers:
        row.append(accuracy_results_test[j][1][name])
    table_data.append(row)

# 표 생성
plt.table(cellText=table_data, colLabels=['Preprocessing Stage', 'Train'] + classifiers + ['Valid'] + classifiers, loc='center')
