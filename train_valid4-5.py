...
# 파라미터 변화에 따른 정확도 기록 함수
def record_parameter_accuracy(param_name, param_values, classifier, x_train, y_train, x_valid, y_valid):
    accuracies = []
    for value in param_values:
        if param_name == 'C':
            clf = LogisticRegression(C=value, max_iter=1000)
        elif param_name == 'max_depth':
            clf = DecisionTreeClassifier(max_depth=value)
        elif param_name == 'hidden_layer_sizes':
            clf = MLPClassifier(hidden_layer_sizes=(value,), max_iter=1000)
...
# 파라미터 값 설정
log_reg_params_part1 = np.arange(0.1, 1.1, 0.2)
log_reg_params_part2 = np.array([1, 10, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000])
log_reg_params = np.concatenate((log_reg_params_part1, log_reg_params_part2))

dec_tree_params = np.linspace(10, 100, 10, dtype=int)
mlp_params = np.linspace(10, 3000, 10, dtype=int)

# 각 인식기별로 파라미터에 따른 정확도 기록
log_reg_accuracies = record_parameter_accuracy('C', log_reg_params, LogisticRegression, x_train_contour, y_train_set, x_valid_contour, y_valid_set)
dec_tree_accuracies = record_parameter_accuracy('max_depth', dec_tree_params, DecisionTreeClassifier, x_train_contour, y_train_set, x_valid_contour, y_valid_set)
mlp_accuracies = record_parameter_accuracy('hidden_layer_sizes', mlp_params, MLPClassifier, x_train_contour, y_train_set, x_valid_contour, y_valid_set)

# 파라미터 변화에 따른 정확도 변화율 그래프 출력
plt.figure(figsize=(15, 5))

# Logistic Regression
plt.subplot(1, 3, 1)
plt.plot(log_reg_params, log_reg_accuracies, marker='o', linestyle='-', color='b')
plt.xscale('log')
plt.xlabel('C value')
...
# Decision Tree
plt.subplot(1, 3, 2)
plt.plot(dec_tree_params, dec_tree_accuracies, marker='o', linestyle='-', color='g')
...
# MLP
plt.subplot(1, 3, 3)
plt.plot(mlp_params, mlp_accuracies, marker='o', linestyle='-', color='r')
